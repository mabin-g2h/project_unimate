...
export async function GET() {
  ...
  try {
    ...
    log({ level: 'error', event: 'peers_fetch_failed', message: 'Failed to load peer directory', userId: session.userId, route: '/api/students/peers', metadata: { error: String(err) } });
    return NextResponse.json({ peers: [] });
  }
}
