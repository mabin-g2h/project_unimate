...
/app
  ...
  /admin/page.tsx                        # Admin review portal (5 tabs: Student Applications, Archives, Manage Dropdowns incl. cities, Manage Admins, Logs). Manage Dropdowns tags each university/city/airport with a country (shown as a chip; ⚠ warning for untagged rows). Review modal's University/City selects are country-scoped (reset + refetch on country change) with an "Other" free-text fallback; a coral "⚠ New — will be added on approval" badge flags a typed university/city not yet in that country's directory. Review modal includes a course_start_date date input (required to approve) with a teal expiry-date preview badge. Student list shows a coral Revoke button for approved students (opens RevokeModal to collect a required reason) and a teal Unrevoke button for revoked students (native confirm dialog); stat boxes include Revoked + Archived counts (the Archived box jumps to the Archives tab); review modal shows revoke_reason as a read-only coral block for revoked students. Archived students are excluded from the Applications list and shown only in the Archives tab — a read-only table (course start / expiry / archived dates + a View button) with an "Archive expired students" button that POSTs to /api/admin/students/archive-expired. Logs tab: newest-first paginated table of student_error_logs (25/page); email search (ILIKE); event chip colour-coded by level (coral=error, amber=warn, blue=info); metadata pills for reason/field/detected_kind; 30-second auto-refresh while tab is open
  /accept-invite/page.tsx                # Admin invite acceptance — set password to activate admin account
  /pending/page.tsx                      # Application status view (pending / rejected / revoked / archived) — revoked students see a gray "Access Suspended" card with a contact-admin message and no re-apply button; archived (expired) students see a gray "Access Ended" card thanking them, with no re-apply button
  ...
  /api/
    /auth/signup/route.ts                # POST — create account, hash password, send verification email; 12-hour verification-link expiry; before the duplicate check purges abandoned student accounts (role='student', verification_expires < NOW(), no student_profiles row) — covers both expired links and verified-but-form-never-submitted accounts — so the email can register fresh
    ...
    /auth/cleanup/route.ts               # GET  — Vercel cron endpoint (Bearer CRON_SECRET); deletes abandoned student accounts (role='student', verification_expires < NOW(), no student_profiles row) — expired verification links AND verified accounts that never submitted the registration form within their 48-hour window; then archives expired approved students via archiveExpiredStudents() (lib/archive.ts) so no second cron job is needed; returns { deleted, archived }; runs daily at midnight UTC
    /auth/verify-email/route.ts          # GET  — validate token; deletes user row if link expired, then redirects; on success sets email_verified=true, clears the token, and (for students) sets verification_expires to a 48-hour deadline to complete the registration form (NULL for admins); sets session and redirects (admin→/admin, student→/register)
    ...
    /student/register/route.ts           # POST — FormData with files + consents_accepted flag; validates consent, inserts student_profiles row with consented_at = NOW(); clears users.verification_expires (the 48-hour form deadline) so the account is never purged; fires admin notification email (fire-and-forget)
    /admin/students/route.ts                          # GET    — list all student applications (admin only); returns revoke_reason, course_start_date, expiry_date, archived_at
    /admin/students/[id]/review/route.ts              # POST   — approve/reject; requires course_start_date to approve (sets course_start_date + expiry_date = course_start_date + ACCOUNT_LIFESPAN_DAYS); deletes passport+admission letter from Blob; sends approval/rejection email; on approve also (a) auto-adds the student's confirmed university+city to the reference tables (best-effort, case-insensitive country-scoped dedup) so dropdowns grow, and (b) notifies all other approved students at the same university (awaited via Promise.allSettled)
    /admin/students/[id]/revoke/route.ts              # POST   — revoke or unrevoke; body { action: 'revoke' | 'unrevoke', reason? }; reason required for revoke — stored in student_profiles.revoke_reason (cleared to NULL on unrevoke); emails student via sendRevokeEmail (coral, with reason) on revoke or sendUnrevokeEmail (brand-blue) on unrevoke (fire-and-forget); validates current status before updating; returns { ok, newStatus }
    /admin/students/[id]/profile/route.ts             # PATCH  — admin edit of a student's profile fields (name, phone, country, university, degree, course, intake, city, course_start_date); re-snapshots expiry_date whenever course_start_date changes
    /admin/students/archive-expired/route.ts          # POST   — admin-triggered sweep; archives every approved student whose expiry_date < NOW() (same logic as the daily cron); calls archiveExpiredStudents() from lib/archive.ts; returns { archived: count }
    /admin/logs/route.ts                              # GET    — paginated student error logs (admin only); ?email= ILIKE filter, ?page= pagination (25/page); returns { logs, total, page }
    ...
    /admin/options/airports/route.ts                  # POST   — create airport with required country (admin only)
    ...
    /options/airports/route.ts                        # GET    — list airports (id, label, country); optional ?country= filter — used to scope the flight-details arrival dropdown (authenticated students)
  ...
  /components/
    ...
    FlightDetailsModal.tsx               # Modal form: departure, arrival, date, airline; exports FlightDetails interface. Departure uses the global airport list; arrival is scoped to the student's country_of_education (accepts a destinationCountry prop, fetches ?country=). Both selects have an "Other — type it in yourself" free-text fallback (stored as-is; no auto-grow)
    ...
    RevokeModal.tsx                      # Admin revoke-reason modal — Apple-style sheet (rm-* CSS classes); required textarea (max 500 chars) + character counter; coral Revoke button disabled until non-empty; loading + inline error state; calls onConfirm(reason) async (parent closes on success, modal shows error inline on failure)
...
/lib/
  db.ts        # neonConfig.fetchFunction patched in development to force IPv4 DNS (local networks with no IPv6 route); exports sql = neon(process.env.DATABASE_CONNECTION_STRING!)
  auth.ts      # signToken, verifyToken, getSession, makeSessionCookieOptions; SessionPayload interface
  email.ts     # sendVerificationEmail, sendRegistrationAcknowledgement, sendAdminRegistrationNotification, sendApprovalEmail, sendRejectionEmail, sendRevokeEmail, sendUnrevokeEmail, sendAccountExpiredEmail, sendAdminInviteEmail, sendPasswordResetEmail, sendNewPeerNotificationEmail, sendPeerInviteEmail
  account.ts   # ACCOUNT_LIFESPAN_DAYS (env or 30-day default), isValidDateString(value), computeExpiry(courseStart) — account lifespan helpers used by review + profile-edit routes
  archive.ts   # archiveExpiredStudents() — UPDATE approved → archived where expiry_date < NOW(); fires sendAccountExpiredEmail per student via Promise.allSettled; shared by the daily cron and the admin on-demand sweep
  ...
  logger.ts    # log(entry) — fire-and-forget INSERT into student_error_logs; LogLevel type ('error'|'warn'|'info'); never throws (`.catch(() => {})`); used across auth, registration, and dashboard routes
...
Tables: `users` (includes `password_reset_token VARCHAR(255)`, `password_reset_expires TIMESTAMPTZ` for the forgot-password flow), `student_profiles` (includes `share_phone BOOLEAN DEFAULT false`, `consented_at TIMESTAMPTZ`, `city VARCHAR(255)` — destination city, nullable for pre-feature rows, `gender VARCHAR(10)`, `revoke_reason TEXT` — set when status → 'revoked', cleared to NULL on unrevoke, `course_start_date DATE` — admin-entered at review (mandatory to approve; NULL for pre-feature rows), `expiry_date DATE` — snapshot = course_start_date + ACCOUNT_LIFESPAN_DAYS, `archived_at TIMESTAMPTZ` — set when status → 'archived'; `status` values: `'pending' | 'approved' | 'rejected' | 'revoked' | 'archived'`), `flight_details`, `student_error_logs` (UUID PK, `level VARCHAR(10)` — 'error'|'warn'|'info', event, message, email as plain text — not FK so pre-account failures are capturable, `user_id` → users ON DELETE SET NULL, route, metadata JSONB, created_at; indexes on created_at DESC, email, (level, created_at DESC)), `universities` (id, name, `country VARCHAR(100)`, created_at; `UNIQUE(name, country)`), `courses` (legacy, unused), `airports` (id, label, `country VARCHAR(100)`, created_at; `UNIQUE(label)` — country scopes the flight arrival dropdown, NULL rows still show as departure options), `airlines`, `cities` (id, label, `country VARCHAR(100)`, created_at; `UNIQUE(label, country)`), `admin_invites` (UUID PK, email, token, expires_at, invited_by → users), `peer_invites` (UUID PK, email, invited_by → users, created_at — no token/expiry; powers per-student rate-limit + dedup + attribution for the Invite-a-peer feature). On `universities`/`cities`, `country` is nullable — pre-feature rows are NULL until re-tagged in admin and won't appear in any country-filtered dropdown.
...
ACCOUNT_LIFESPAN_DAYS        # Optional: days after course_start_date before access expires (default: 30); changing only affects future approvals — expiry_date is snapshotted in DB
```
...
- **Reference-table `country` is nullable and re-tag-only** — `universities`/`cities`/`airports` rows created before the country-scoping feature have `country = NULL`, so they appear in no country-filtered dropdown until an admin sets a country (for airports this means they still show as departure options but never as arrival options). There is no inline edit on these rows — admins delete and re-add with a country (shown by a ⚠ "No country — re-add" chip in Manage Dropdowns).
- **`course_start_date` / `expiry_date` nullable for pre-feature rows** — students approved before the account-expiry feature have `course_start_date = NULL` and `expiry_date = NULL`; the archive sweep ignores them (`WHERE expiry_date IS NOT NULL`). An admin can backfill `course_start_date` retroactively via the review-modal profile edit (PATCH `/api/admin/students/[id]/profile`), which re-snapshots `expiry_date` and makes the row eligible for future archiving.

## Phase 1 Scope — What Exists

- Signup → email verification (12-hour link expiry) → on verify, students get a 48-hour deadline to submit the registration form → login/logout (with sign-out button in Navbar). Abandoned student accounts (expired link, or verified but form never submitted within 48h, with no student_profiles row) are auto-deleted by the daily cron + inline on the next signup with that email, freeing it to register fresh. Submitting the form clears the deadline; admins never get one.
...
- Flight details form: students enter their flight info. Departure uses the full global airport list; **arrival is scoped to the student's `country_of_education`** (filtered via `?country=`). Both fields have an "Other — type it in yourself" free-text fallback so a student is never blocked when their airport isn't configured; typed values are stored as-is and are **not** auto-added to the airports table. Peer cards show a **"Same flight!"** badge when travel date + departure + arrival + airline all match the viewer, **"Same day"** when only the date matches
...
- Self-growing directory: when an admin **approves** a student, the student's (admin-confirmed) university and city are auto-added to the reference tables, tagged with their `country_of_education`. The insert is best-effort (never blocks approval) and uses a case-insensitive, country-scoped existence check + `ON CONFLICT DO NOTHING` to avoid near-duplicate rows. The review modal shows a coral "⚠ New — will be added…" badge when a value isn't yet in that country's directory. Airports are **not** auto-grown (no admin-review gatekeeper on flight details) — admins add them manually in Manage Dropdowns, tagged with a country.
...
- Admin revoke/unrevoke: approved students can have their access suspended at any time via a Revoke button in the student list (coral); clicking it opens `RevokeModal` where the admin must enter a required reason (1–500 chars). On confirm the reason is stored in `student_profiles.revoke_reason` and emailed to the student (`sendRevokeEmail`, fire-and-forget). A revoked student is redirected to `/pending` showing a gray "Access Suspended" card and is hidden from all peer directories (peers query filters `status = 'approved'`). Unrevoke (teal, native confirm dialog) restores access immediately and emails the student (`sendUnrevokeEmail`); `revoke_reason` is cleared to NULL. Revoke reason is shown as a read-only coral block in the review modal for revoked students. Implemented via `POST /api/admin/students/[id]/revoke`
- Account expiry + archive: when an admin approves a student they must enter a `course_start_date` (validated `YYYY-MM-DD`); `expiry_date` is computed as `course_start_date + ACCOUNT_LIFESPAN_DAYS` and snapshotted in the DB at approval. When expiry passes, students are moved from `status='approved'` to `status='archived'` and stamped `archived_at`; `sendAccountExpiredEmail` is sent to each. The sweep runs via the daily cleanup cron **and** via `POST /api/admin/students/archive-expired` (admin-on-demand button). Archived students are hidden from peer directories (same `status='approved'` filter). `course_start_date` can be updated retroactively via the profile-edit PATCH, which re-snapshots `expiry_date`. Shared logic lives in `lib/archive.ts`; lifespan constants + date helpers in `lib/account.ts`
- Student error logging: auth, registration, and dashboard route failures are captured fire-and-forget into `student_error_logs` via `lib/logger.ts`. Events have a `level` (`error`=system failure, `warn`=user mistake, `info`=soft block). Dashboard routes (`/api/students/peers`, `flight`, `share-phone`) degrade gracefully on DB failure (return empty data rather than 500) and log the error. The admin Logs tab shows all entries with email search, level-coded event chips, metadata pills, and 30-second auto-refresh
...
Career hub, PR calculator, job board, marketplace, events ticketing, senior mentors, Google OAuth, in-app messaging, automated OCR, AWS S3/SES migration, Prisma migration, setup-password JWT flow, affiliate consents.
